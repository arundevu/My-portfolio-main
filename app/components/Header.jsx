...
const Header = ({ isDarkMode }) => {
    return (
        <div className='w-11/12 max-w-3xl text-center mx-auto h-screen flex flex-col items-center justify-center gap-4'>
            ...
            >
                <Image src={assets.arun_image} alt='profile' className='rounded-full w-80 mt-8' />
            </motion.div>
            <motion.h3 className='flex items-end gap-2 text-xl md:text-2xl mb-3 font-Ovo'
                initial={{ y: -20, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.8, delay: 0.3 }}>
                Hi I'm Arun Devu <Image src={assets.hand_icon} alt='hand' className='w-6' /></motion.h3>
            ...
            <div className='flex flex-col sm:flex-row items-center gap-4 mt-4'>
                <motion.a
                    ...
                    href="#contact" className={`px-10 py-3 border border-white rounded-full bg-black text-white flex items-center gap-2 ${isDarkMode ? "dark:bg-transparent" : ""}`}> Contact me
